 ...
 * Copyright (C) 2017 - 2025 Olivier Sessink
 *
...
{
	convert_indenting(BFWIN(user_data)->current_document, FALSE);
}

static void
ui_indenting_to_tabs(GtkAction * action, gpointer user_data)
{
	convert_indenting(BFWIN(user_data)->current_document, TRUE);
}
	...
	if (gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action)) && bfwin->current_document) {
		...
		if (doc->uri && oldmime && strchr(oldmime, '?') == NULL) {
			...
			DEBUG_MSG("lang_mode_menu_activate, got %p for tmpbflang with name %s\n", tmpbflang,
					  tmpbflang?tmpbflang->name:"Undefined");
...
bfwin_main_ui_init(Tbfwin * bfwin, GtkWidget * vbox)
{
	...
	if (main_v->props.dynamic_menu_shortcut) {
		gtk3_enable_dynamic_menu_shortcuts(bfwin->menubar, NULL);
	}
#endif
#endif
...
#ifndef MAC_INTEGRATION
	if (main_v->props.dynamic_menu_shortcut) {
		gchar *newpath = g_strconcat(path, "/", action_name, NULL);
		GtkWidget *menuitem = gtk_ui_manager_get_widget(uimanager, newpath);
		if (menuitem)
			gtk3_enable_dynamic_menu_shortcuts_menuitem(menuitem, NULL);
		g_free(newpath);
	}
#endif
#endif
