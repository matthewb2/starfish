 ...
 *
 * Copyright (C) 2002-2025 Olivier Sessink
...
preferences_apply(Tprefdialog * pd)
{
	...
	integer_apply(&main_v->props.dynamic_menu_shortcut, pd->prefs[dynamic_menu_shortcut], TRUE);
#endif
...
preferences_dialog_new(Tbfwin *bfwin)
{
	...
	pd->prefs[dynamic_menu_shortcut] = dialog_check_button_new(_("Enable dynamic menu shortcuts - move your mouse over a menu entry and press the new shortcut key combination"),
															 main_v->props.dynamic_menu_shortcut);
	gtk_box_pack_start(GTK_BOX(vbox2), pd->prefs[dynamic_menu_shortcut], FALSE, FALSE, 0);
