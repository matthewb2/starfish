...
gint doc_offset_from_line(Tdocument *doc, gint line) {
	if (!doc || !doc->buffer) return 0;
	GtkTextIter iter;
	gtk_text_buffer_get_iter_at_line(doc->buffer, &iter, line-1);
 ...
 * @uri a GFile * uri
 *
 * if the file is open, it returns the Tdocument* 
 * if the file is not open it returns NULL
 *
 * Return value: #Tdocument* or NULL if not open
 **/
Tdocument *
documentlist_return_document_from_uri(GFile * uri)
{
...
{
	GtkWrapMode wmode;
	g_print("doc_set_wrap(doc=%p, enabled=%d)\n",doc,enabled);
	if (enabled) {
...
/* here we ask any plugins to add their menu item */
	if (main_v->doc_view_populate_popup_cbs) {
		GSList *tmplist = main_v->doc_view_populate_popup_cbs;
		while (tmplist) {
			void *(*func) (GtkTextView *, GtkMenu *, Tdocument *) = tmplist->data;
...
doc_recalculate_right_margin(Tdocument * doc)
{
	GdkWindow *gwin;
	gint width = -1;
	DEBUG_MSG("doc_recalculate_right_margin(doc=%p); with main_v->props.wrap_on_right_margin=%d\n",doc,main_v->props.wrap_on_right_margin);
	...
	DEBUG_MSG("doc_recalculate_right_margin, width=%d, wrapmode=%d\n",width, gtk_text_view_get_wrap_mode(GTK_TEXT_VIEW(doc->view)));
	if (width >= 0 && gtk_text_view_get_wrap_mode(GTK_TEXT_VIEW(doc->view)) != GTK_WRAP_NONE) {
		gint margin = width - BLUEFISH_TEXT_VIEW(doc->view)->margin_pixels_per_char *	main_v->props.right_margin_pos;
		main_v->props.adv_textview_right_margin = margin;
		DEBUG_MSG("doc_recalculate_right_margin, calculate and set right margin %d\n",margin);
		gtk_text_view_set_right_margin(GTK_TEXT_VIEW(doc->view),margin);
		if (doc->slave)
			gtk_text_view_set_right_margin(GTK_TEXT_VIEW(doc->slave),margin);
	} else { /* either there is no window width known, or wrap is disabled */
		DEBUG_MSG("doc_recalculate_right_margin, use adv_textview_right_margin %d\n",main_v->props.adv_textview_right_margin);
...
doc_activate(Tdocument * doc)
{
	...
	if (BFWIN(doc->bfwin)->recentdoclist != doc->recentpos) {
		...
		BFWIN(doc->bfwin)->recentdoclist = g_list_remove_link(BFWIN(doc->bfwin)->recentdoclist, doc->recentpos);
