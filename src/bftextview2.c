...
bftextview2_get_iter_at_bevent(BluefishTextView * btv, GdkEventButton * bevent, GtkTextIter * iter)
{
	...
	gtk_text_view_get_iter_at_position (GTK_TEXT_VIEW(btv), iter, &trailing, xpos, ypos);
}
...
static gboolean bluefish_text_view_button_press_event(GtkWidget * widget, GdkEventButton * event)
{
	...
	if (gtk_text_view_get_window(GTK_TEXT_VIEW(btv), GTK_TEXT_WINDOW_TEXT)) {
		GtkTextIter iter;
		...
		GtkTextIter mstart, position, matchso, matcheo;
		guint matchnum=0;
		gint contextnum;
		...
		if (master->bflang && master->bflang->st) {
			...
			if (matchnum) {
				if (g_array_index(master->bflang->st->matches, Tpattern, matchnum).is_color) {
					 /* pass to the plugin that this is a color */
					 g_print("clicked in a color for match with pattern %s\n",g_array_index(master->bflang->st->matches, Tpattern, matchnum).pattern);
				} else {
					g_print("clicked in pattern %d in context %d with pattern %s\n",matchnum,contextnum,g_array_index(master->bflang->st->matches, Tpattern, matchnum).pattern);
				}
			}
		}
	
		/* here we ask any plugins to do any processing, used by the htmlbar plugin to see if there is a color or tag on the button location */
		if (main_v->doc_view_button_press_cbs) {
			GSList *tmplist = main_v->doc_view_button_press_cbs;
			while (tmplist) {
				void *(*func) (GtkWidget *, GdkEventButton *, Tdocument *, GtkTextIter *position, GtkTextIter *so, GtkTextIter *eo, gboolean is_color, gchar *pattern) = tmplist->data;
				DEBUG_MSG
				...
				func(widget, event, (Tdocument *) master->doc, 
							&position,&matchso, &matcheo,
							matchnum?g_array_index(master->bflang->st->matches, Tpattern, matchnum).is_color:FALSE,
							matchnum?g_array_index(master->bflang->st->matches, Tpattern, matchnum).pattern:NULL
							);
								 ...
								 GtkTooltip * tooltip)
{
	...
	if (master->bflang && master->bflang->st && master->enable_scanner && master->scanner_idle == 0) {
		...
		if (matchnum) {
			if (main_v->props.show_tooltip_reference && g_array_index(master->bflang->st->matches, Tpattern, matchnum).reference) {
			...
			if (g_array_index(master->bflang->st->matches, Tpattern, matchnum).is_color) {
				...
				gchar *str = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(btv->buffer), &mstart, &iter,FALSE);
				GdkPixbuf *pbuf = pixbuf_new_for_color(str, &red, &green, &blue);
				g_free(str);
				if (pbuf) {
					gchar *hexcolorstring;
					gtk_tooltip_set_icon(tooltip, pbuf);
					g_object_unref(G_OBJECT(pbuf));
					/*g_print("building tooltip for r=%d,g=%d,b=%d\n",red,green,blue);*/
					hexcolorstring = g_strdup_printf("#%02X%02X%02X",(int)red,(int)green,(int)blue); 
					str = g_strdup_printf("<span fgcolor=\"%s\" bgcolor=\"black\"> rgb(%d,%d,%d) </span>\n<span fgcolor=\"%s\" bgcolor=\"white\"> rgb(%d,%d,%d) </span>\n<span fgcolor=\"white\" bgcolor=\"%s\"> %s </span>\n<span fgcolor=\"black\" bgcolor=\"%s\"> %s </span>"
								,hexcolorstring
					...
					/*g_print("building tooltip str=%s\n",str);*/
					gtk_tooltip_set_markup(tooltip,str);
					g_free(str);
					g_free(hexcolorstring);
					return TRUE;
				}
				return FALSE;
			}
