...
{
	GtkWidget *alignment, *carea, *table, *vbox, *vbox2;
	Tfiles_advanced *tfs;
						  ...
						  gpointer callback_data)
{
	...
	case CHECKMODIFIED_ERROR:
		...
		if (gerror->code == G_IO_ERROR_NOT_FOUND) {
			gchar *tmpstr;
			gint retval;
			const gchar *buttons[] = { _("_Unset file name"), _("_Save"), _("Retry later"), NULL };
			...
			} else if (retval == 2){			/* unset */
				document_unset_filename(doc);
			} else { /* retry later, this is useful if a network connection is temporarily unavailable */
