 ...
 *
 * Copyright (C) 2005-2024 Olivier Sessink
 *
...
child_watch_lcb(GPid pid, gint status, gpointer data)
{
	...
	if (ep->statuscode_cb) {
		ep->statuscode_cb(status, BFWIN(ep->bfwin), ep->commandstring, ep->data);
	}
...
static gboolean
per_line_io_watch_lcb(GIOChannel * channel, GIOCondition condition, gpointer data)
{
	Texternalp *ep = data;
	if (!ep->customcommand_cb)
		return FALSE;
	DEBUG_MSG("per_line_io_watch_lcb, called with condition %d\n", condition);
	if (condition & G_IO_IN) {
		...
		while (status == G_IO_STATUS_NORMAL) {
			if (buflen > 0) {
				if (termpos < buflen)
					buf[termpos] = '\0';
				ep->customcommand_cb(buf, ep->bfwin, ep->data);
				ep->havedata = TRUE;
			}
			g_free(buf);
		...
		if (gerror) {
			g_warning("error while trying to read data for per_line, %d: %s\n", gerror->code,
					  gerror->message);
			g_error_free(gerror);
		}
	...
	if (condition & G_IO_OUT) {
		DEBUG_MSG("per_line_io_watch_lcb, condition %d G_IO_OUT not handled\n", condition);
	}
	if (condition & G_IO_PRI) {
		DEBUG_MSG("per_line_io_watch_lcb, condition %d G_IO_PRI not handled\n", condition);
	}
	if (condition & G_IO_ERR) {
		DEBUG_MSG("per_line_io_watch_lcb, condition %d G_IO_ERR not handled\n", condition);
	}
	if (condition & G_IO_HUP) {
		GError *error = NULL;
		DEBUG_MSG("per_line_io_watch_lcb, condition %d G_IO_HUP\n", condition);
		/* cleanup !!!!!!!!! */
		g_io_channel_shutdown(channel, TRUE, &error);
		externalp_unref(ep);
		return FALSE;
	...
	if (condition & G_IO_NVAL) {
		DEBUG_MSG("per_line_io_watch_lcb, condition %d G_IO_NVAL not handled\n", condition);
	}
	return TRUE;
}
...
void
generic_statuscode_lcb(gint status, Tbfwin * bfwin, const gchar *commandstring, gpointer data)
{
	DEBUG_MSG("generic_statuscode_lcb, exitcode %d\n", status);
#ifndef WIN32
	/* on windows this even fires dialogs when opening something in firefox, disabling that for the moment */
	if (status != 0) {
		gchar *tmp =
			g_markup_printf_escaped(_("The command %s exited with error code %d. %s"), commandstring,
									status,
									(status ==
									 32512) ? _("Probably this application is not installed on your system.")
									: "");
		message_dialog_new(BFWIN(bfwin)->main_window, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
...
filter_custom_lcb(const gchar * str_return, gpointer bfwin, gpointer data)
{
	...
	bmark_clean_for_doc(BFWIN(bfwin)->current_document);
	doc_replace_text(BFWIN(bfwin)->current_document, str_return, ep->begin, end);
	bmark_set_for_doc(BFWIN(bfwin)->current_document, TRUE);
	if (line != -1) {
...
{
	command_backend(bfwin, begin, end, formatstring, full_data_io_watch_lcb, FALSE, generic_statuscode_lcb,
...
{
	command_backend(bfwin, 0, -1, formatstring, full_data_io_watch_lcb, FALSE, generic_statuscode_lcb, external_custom_lcb, NULL);
}
...
/* same as custom_command(), except it calls CustomCommandCallback for each line */
void
custom_command_lines(Tbfwin * bfwin, const gchar * formatstring, StatusCodeCallback statuscode_cb, CustomCommandCallback customcommand_cb, gpointer data)
{
	command_backend(bfwin, 0, -1, formatstring, per_line_io_watch_lcb, TRUE, statuscode_cb, customcommand_cb, data);
}
