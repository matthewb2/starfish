 ...
 * modifications (C) 2004-2024 Olivier Sessink
...
So they store the same Gtktreestore as well. 

When a project is open, it has it's own bookmarks, so they should be stored
separately. The project functions create a new gtktreestore when they convert 
a window (Tbfwin) into a project window. 
...
typedef struct {
	GtkTextMark *mark;
	GFile *uri;
	gint offset; /* offset in chracters in the document - except if bit 31 is set, then this is the offset in lines in the document */
	Tdocument *doc;
...
static void bmark_get_iter_at_tree_position_and_show(Tbfwin * bfwin, Tbmark * m, const gchar *color);
 ...
 */
static Tbmark *
bmark_find_bookmark_before_offset(Tbfwin * bfwin, guint offset, GtkTreeIter * parent, gboolean do_update)
{
	gboolean cont;
	GtkTreeIter iter;
	Tbmark *b1 = NULL, *b2;
	...
	cont =
		gtk_tree_model_iter_children(GTK_TREE_MODEL(BMARKDATA(bfwin->bmarkdata)->bookmarkstore), &iter,
									 parent);
	while (cont) {
		gtk_tree_model_get(GTK_TREE_MODEL(BMARKDATA(bfwin->bmarkdata)->bookmarkstore), &iter, PTR_COLUMN, &b2,-1);
		if (do_update)
...
bmark_doc_renamed(Tbfwin * bfwin, Tdocument * doc, GFile *copy_bookmarks_uri)
{
	...
	for (tmplist = g_list_first(copylist);tmplist;tmplist=tmplist->next) {
		...
		bmark_get_iter_at_tree_position_and_show(bfwin, b, NULL);
	}
...
static void 
bmark_select_in_treeview(Tbfwin *bfwin, Tbmark *b) {
	GtkTreeIter fiter;
	if (!b) return;
	DEBUG_MSG("bmark_select_in_treeview, select the iter for the bmark in the treeview\n");
...
bmark_activate(Tbfwin * bfwin, Tbmark * b, gboolean select_bmark)
{
	...
	if (b->doc) {
		...
		/* it could be that the offset is a line number, this can be seen if bit 31 in offset is set */
		if (b->offset & (1<<31)) {
			doc_new_from_uri(bfwin, b->uri, NULL, FALSE, FALSE, -1, b->offset, -1, TRUE, FALSE);
		} else {
			DEBUG_MSG("bmark_activate, calling doc_new_from_uri with goto_line %d\n", b->offset&~(1<<31));
			doc_new_from_uri(bfwin, b->uri, NULL, FALSE, FALSE, (b->offset &~(1<<31)), -1, -1, TRUE, FALSE);
		}
	}
	/* remove selection */
	if (b->doc) {
	...
	if (select_bmark)
		bmark_select_in_treeview(bfwin, b);
}
...
static void
bmark_del_children_backend(Tbfwin * bfwin, GtkTreeIter * parent, gboolean onlytemp)
{
	GtkTreeIter tmpiter;
	gboolean cont=TRUE;
	cont = gtk_tree_model_iter_children(GTK_TREE_MODEL(BMARKDATA(bfwin->bmarkdata)->bookmarkstore),&tmpiter, parent);
	while (cont) {
		Tbmark *b;
		...
		cont = gtk_tree_model_iter_next(GTK_TREE_MODEL(BMARKDATA(bfwin->bmarkdata)->bookmarkstore),&tmpiter);
		if (b && (onlytemp == FALSE || b->is_temp == TRUE)) {
			DEBUG_MSG("bmark_del_children_backend, delete b=%p with name %s\n", b, b->name);
			gboolean parentstillthere = bmark_check_remove(bfwin, b);
			DEBUG_MSG("bmark_del_children_backend, after delete b=%p, cont=%d\n", b, cont);
			if (!b->is_temp)
				bmark_unstore(bfwin, b);
			bmark_free(b);
			if (!parentstillthere)
...
static void
popup_menu_follow_cursor(GtkAction * action, gpointer user_data)
{
	main_v->globses.bookmarks_follow_cursor = gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action));
}
...
popup_menu_delete_all(GtkAction * action, gpointer user_data)
{
	...
	while (gtk_tree_model_iter_children
		...
		bmark_del_children_backend(bfwin, &tmpiter, FALSE);

	gtk_widget_grab_focus(bfwin->current_document->view);
}

static gboolean 
get_parent_iter_from_cursor(Tbfwin *bfwin, GtkTreeIter *iter) {
	GtkTreePath *path;
	GtkTreeViewColumn *col;
	gint depth;
	GtkTreeModel *model;
	...
	model = gtk_tree_view_get_model(bfwin->bmark);
	gtk_tree_model_get_iter(model, &filteriter, path);
	gtk_tree_path_free(path);
	gtk_tree_model_filter_convert_iter_to_child_iter(GTK_TREE_MODEL_FILTER(model), iter, &filteriter);
	return TRUE;
}
...
popup_menu_delete_all_doc(GtkAction * action, gpointer user_data)
{
	...
	if (get_parent_iter_from_cursor(bfwin, &iter)) {
		const gchar *buttons[] = { GTK_STOCK_NO, GTK_STOCK_YES, NULL };
		gchar *name=NULL, *pstr;
		gint retval;
		...
		if (retval == 0)
			return;
		bmark_del_children_backend(bfwin, &iter, FALSE);
	}
...
static void
popup_menu_delete_temp_doc(GtkAction * action, gpointer user_data)
{
	Tbfwin *bfwin = BFWIN(user_data);
	GtkTreeIter iter;
	if (!bfwin->bmark)
	...
	if (get_parent_iter_from_cursor(bfwin, &iter)) {
		bmark_del_children_backend(bfwin, &iter, TRUE);
	}
}
...
static const GtkActionEntry bookmark_actions[] = {
	...
	N_("Delete all bookmarks in document"),G_CALLBACK(popup_menu_delete_all_doc)},
	{"DeleteTempBookmarkInDoc", NULL, N_("Delete temporary in Document"), NULL,
	N_("Delete temporary bookmarks in document"),G_CALLBACK(popup_menu_delete_temp_doc)}
};
...
static const GtkToggleActionEntry bookmark_toggle_actions[] = {
	...
	{"FollowCursor", NULL, N_("Follow cursor"), NULL, N_("Follow cursor position"),
	G_CALLBACK(popup_menu_follow_cursor)}
};
...
popup_menu_action_group_init(Tbfwin * bfwin)
{
	...
	gtk_ui_manager_add_ui_from_string(bfwin->uimanager, bookmark_menu_ui, -1, NULL);
	gtk_ui_manager_add_ui_from_string(bfwin->uimanager, bookmark_search_menu_ui, -1, NULL);
}

static void
extbookmark_menu_activate_lcb(GtkAction * action, gpointer user_data)
{
	Tbfwin *bfwin = BFWIN(user_data);
	const gchar **arr = g_object_get_data(G_OBJECT(action), "adata");
	bookmark_command_from_arr(bfwin, arr);
}


static void
popup_menu_generate_init(Tbfwin *bfwin) {
	if (!bfwin->extbookmark_group) {
		bfwin->extbookmark_group = gtk_action_group_new("ExtbookmarkActions");
		gtk_ui_manager_insert_action_group(bfwin->uimanager, bfwin->extbookmark_group, 1);
	} else {
		dynamic_menu_empty(bfwin->uimanager, bfwin->extbookmark_merge_id, bfwin->extbookmark_group);
	}

	bfwin->extbookmark_merge_id = gtk_ui_manager_new_merge_id(bfwin->uimanager);
	for (GList *list = g_list_last(main_v->props.external_bookmarkcommands); list; list = list->prev) {
		gchar **arr = (gchar **) list->data;
		if (g_strv_length(arr) == 8 && arr[0] && (arr[0][0] == '1' || arr[0][0] == '3') && arr[1]) {
			gchar *action_name = g_strconcat("extbookmark", arr[1], NULL);
			dynamic_menu_item_create(bfwin->uimanager, bfwin->extbookmark_group,
									 bfwin->extbookmark_merge_id,
									 "/BookmarkMenu/ExtcommandPlaceholder", action_name,
									 arr[1], -1, NULL, G_CALLBACK(extbookmark_menu_activate_lcb), bfwin, arr);
			g_free(action_name);
		}
	}
}
...
popup_menu(Tbfwin * bfwin, GdkEventButton * event, gboolean show_bmark_specific, gboolean show_file_specific)
{
	...
	if (!menu)
		return;
	popup_menu_generate_init(bfwin);
...
{
	g_print("bmark_search_icon_press, entry=%p, primary=%d, secondary=%d\n",entry, (icon_pos == GTK_ENTRY_ICON_PRIMARY), (icon_pos == GTK_ENTRY_ICON_SECONDARY));
...
bmark_previous_lcb(GtkWidget * widget, Tbfwin * bfwin)
{
	...
	line = bmark_margin_get_initial_bookmark(CURDOC(bfwin), gtk_text_iter_get_offset(&titer), (gpointer) & bmark);
...
bmark_search_changed(GtkEditable * editable, gpointer user_data)
{
	...
	if (strlen(bfwin->bmark_search_prefix)>1) {
...
bmark_gui(Tbfwin * bfwin)
{
	...
	gtk_entry_set_icon_from_stock(GTK_ENTRY(entry), GTK_ENTRY_ICON_SECONDARY, GTK_STOCK_DELETE);
	...
	column = gtk_tree_view_column_new_with_attributes("", cell, "text", NAME_COLUMN, "foreground", COLOR_COLUMN, NULL);
 ...
 */
static void
bmark_get_iter_at_tree_position_and_show(Tbfwin * bfwin, Tbmark * m, const gchar *color)
{
	GtkTreeIter *parent;
	gpointer ptr;
	gchar *displaytext;
	...
	gtk_tree_store_set(BMARKDATA(bfwin->bmarkdata)->bookmarkstore, &m->iter, NAME_COLUMN,
					   displaytext, PTR_COLUMN, m, COLOR_COLUMN, color,-1);
	g_free(displaytext);
}
...
bookmark_data_new(void)
{
	...
	bmd->bookmarkstore = gtk_tree_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_STRING);
...
this is also called from external_commands.c before a filter is about to replace all of the contents of a document
 */
void
 ...
 * this is called from file.c after the contents of a file are loaded 
 * this is also called from external_commands.c after an external filter has replaced the contents of a file
 *
...
/*	if (!BFWIN(doc->bfwin)->bmark) {
	...
	while (cont2) {				/* loop the bookmarks for this document  */
		...
		if (mark) {
			...
			if (mark->offset & 1<<31) {
				g_print("offset was %d, so this is a line, set iter at line %d\n",mark->offset, mark->offset&~(1<<31));
				/* if bit 31 is set, this bookmark was never loaded before, and the offset is a line number */
				gtk_text_buffer_get_iter_at_line(doc->buffer, &it, mark->offset&~(1<<31));
				...
				if (check_positions && bookmark_needs_repositioning(mark, &it)) {	/* repositioning required ! */
					if (bookmark_reposition(mark, gtk_text_buffer_get_char_count(doc->buffer) - mark->len)) {
						...
						g_warning("failed to restore bookmark with text '%s'\n",mark->text);
					}
				}
			}
...
bmark_margin_get_initial_bookmark(Tdocument * doc, guint offset, gpointer * bmark)
{
	...
	*bmark = bmark_find_bookmark_before_offset(BFWIN(doc->bfwin), offset, doc->bmark_parent, FALSE);	/* returns NULL if there is no existing bookmark *before* offset */
	if (!*bmark) {
		GtkTreeIter treeit;
		gboolean retval =
			gtk_tree_model_iter_children(GTK_TREE_MODEL
 ...
 *
 * will use doc if it is set, otherwise uri must be set
 */
static Tbmark *
bmark_add_backend(Tbfwin *bfwin, Tdocument * doc, GFile *uri, GtkTextIter * itoffset, guint offset, const gchar * name,
				  const gchar * text, gboolean is_temp, const gchar *color)
{
	Tbmark *m;
	GtkTextIter it;
	...
	if (!doc) {
		// see if a document with this uri is open
		doc = g_hash_table_lookup(main_v->alldochash, uri);
	}
	
	if (doc) {
		m->doc = doc;
		m->uri = g_object_ref(doc->uri);
		if (itoffset) {
			it = *itoffset;
		...
		m->uri = g_object_ref(uri);
		m->offset = offset;
	}

	DEBUG_MSG("bmark_add_backend, Tbmark=%p, create GtkTextMark %p\n", m, m->mark);
	m->is_temp = is_temp;
	m->text = text?g_strdup(text):NULL;
	m->name = (name) ? g_strdup(name) : g_strdup("");
	m->description = g_strdup("");

	/* insert into tree */
	bmark_get_iter_at_tree_position_and_show(bfwin, m, color);

	/* and store */
	if (!m->is_temp) {
		bmark_store(BFWIN(bfwin), m);
	}
	if (doc) {
...
bmark_add_current_doc_backend(Tbfwin * bfwin, const gchar * name, gint offset, gboolean is_temp)
{
	...
	if (!bfwin->current_document)
		return;
	DEBUG_MSG("bmark_add_current_doc_backend, adding bookmark at offset=%d for bfwin=%p\n", offset, bfwin);
	/* create bookmark */
	...
	if (gtk_text_buffer_get_selection_bounds(DOCUMENT(bfwin->current_document)->buffer, &sit, &eit)
		...
		text = gtk_text_iter_get_text(&sit, &eit);
	} else {
		text = bmark_text_for_offset(DOCUMENT(bfwin->current_document), &it, offset);
	}
	m = bmark_add_backend(bfwin, DOCUMENT(bfwin->current_document), DOCUMENT(bfwin->current_document)->uri, &sit, offset, name, text, is_temp, is_temp?"#666666":NULL);
	g_free(text);
...
bmark_get_bmark_at_offset(Tdocument * doc, gint offset)
{
	...
	/* the next function is probably the slowest since it jumps through the listmodel 
	   to find the right bookmark, AND it updates all the bookmark offsets */
	m = bmark_find_bookmark_before_offset(BFWIN(doc->bfwin), offset, doc->bmark_parent, TRUE);
	if (m == NULL) {
 ...
 * this function.
 *
 * called by snr3.c and bookmark_command.c
 */
void
bmark_add_extern(Tbfwin *bfwin, Tdocument * doc, gint offset, const gchar * name, const gchar * text, gboolean is_temp, const gchar *color)
{
	if (!doc)
		return;
	DEBUG_MSG("bmark_add_extern, adding bookmark at offset %d with name %s\n", offset, name);	/* dummy */
	if (text) {
		bmark_add_backend(bfwin, doc, NULL, NULL, offset, (name) ? name : "", text, is_temp, color);
	} else {
		gchar *tmp = bmark_text_for_offset(doc, NULL, offset);
		bmark_add_backend(bfwin, doc,NULL, NULL, offset, (name) ? name : "", tmp, is_temp, color);
		g_free(tmp);
	}
}

/* 
 * this must be a temp bookmark, otherwise you can add a bookmark for a non-existing file that can never be opened
 * (which is required to convert the line number to an offset, and to fill in the 'text'
*/
void
bmark_add_for_line_on_file_extern(Tbfwin *bfwin, GFile *uri, guint line, const gchar * name, const gchar *color)
{
	if (!uri)
		return;
	
	Tdocument *tempdoc;
	tempdoc = documentlist_return_document_from_uri(uri);
	if (tempdoc) {
		GtkTextIter iter;
		gtk_text_buffer_get_iter_at_line(GTK_TEXT_BUFFER(tempdoc->buffer),&iter,line-1);
		gchar *text = bmark_text_for_offset(tempdoc, &iter, 0);
		bmark_add_backend(bfwin, tempdoc, NULL, &iter, 0, name, text, TRUE, color);
		g_free(text);
		DEBUG_MSG("bmark_add_for_file_extern, adding bookmark for doc %p at line %d with name %s\n", tempdoc, line, name);
	} else {
		/* add for a file that is not open, set bit 31 to indicate this is a line number!!! */
		DEBUG_MSG("bmark_add_for_file_extern, adding bookmark for file that is not open at line %d with name %s\n", line, name);
		bmark_add_backend(bfwin, NULL, uri, NULL, (line-1) | (1 << 31), name, NULL, TRUE, color);
	}
}
...
bmark_toggle(Tdocument * doc, gint offset, const gchar * name, const gchar * text)
{
	...
	if (bmark) {
		...
		bmark_add_extern(BFWIN(doc->bfwin), doc, offset, name, text, !main_v->globses.bookmarks_default_store, NULL);
	}
}
...
void
bmark_sync_bmark_cursor_position(Tdocument *doc, guint cursoroffset) {
	if (!doc || !doc->bmark_parent) return;
	if (main_v->globses.bookmarks_follow_cursor) {
		Tbmark *bmark = bmark_find_bookmark_before_offset(doc->bfwin, cursoroffset, doc->bmark_parent, FALSE);
		if (bmark)
			bmark_select_in_treeview(doc->bfwin, bmark);
	}
}
