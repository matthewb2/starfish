...
process_scanning_element(xmlTextReaderPtr reader, Tbflangparsing * bfparser, gint16 context,
						 ...
						 gint ih_case_insens, gint ih_is_regex, gint ih_is_color, GSList *ih_autocomplete)
{
	guint16 matchnum = 0;
	...
	gint case_insens = UNDEFINED, is_regex = UNDEFINED, is_color = UNDEFINED;
	...
	Tattrib attribs[] = {{"pattern", &pattern, attribtype_string},
					...
					{"is_color", &is_color, attribtype_boolean},
	...
	if (add_element) {
		...
		if (!used_idref && pattern && pattern[0]) {
			...
			matchnum = add_pattern_to_scanning_table(bfparser->st,
								...
								is_color != UNDEFINED ? is_color : (ih_is_color !=UNDEFINED ? ih_is_color : FALSE),
...
create_attribute_add_string(Tbflangparsing * bfparser, guint16 contexttag, const gchar *stringname, const gchar *singlematch, const gchar *fullmatch)
{
	...
	if (strcontext == 0) {
		...
		endmatch = add_pattern_to_scanning_table(bfparser->st, singlematch, FALSE, FALSE, FALSE, strcontext, &bfparser->ldb);
		pattern_set_runtime_properties(bfparser->st, endmatch, stringhighlight,-1,FALSE,FALSE,0,FALSE,FALSE);
	}
	matchstring = add_pattern_to_scanning_table(bfparser->st, singlematch, FALSE, FALSE, FALSE, contexttag, &bfparser->ldb);
					...
					, const gchar *attrib_autocomplete_append, gint attrib_autocomplete_backup_cursor)
{
	...
	if (strchr(attrstring, '=')== NULL) {
		...
		attrmatch = add_pattern_to_scanning_table(bfparser->st, attrstring, FALSE, FALSE, TRUE, contexttag, &bfparser->ldb);
		...
		attrmatch = add_pattern_to_scanning_table(bfparser->st, tmp, TRUE, FALSE, TRUE, contexttag, &bfparser->ldb);
		g_free(tmp);
...
attribute_add_value(Tbflangparsing * bfparser, gchar *string, guint16 valuecontext)
{
	...
	valmatchnum = add_pattern_to_scanning_table(bfparser->st,string,FALSE,FALSE,TRUE, valuecontext, &bfparser->ldb);
		...
		if (attribmatchnum) {
			...
			if (values && values[0]) {
				...
				attribmatchnum = add_pattern_to_scanning_table(bfparser->st,
										pattern,TRUE,FALSE,TRUE,tagattributecontext, &bfparser->ldb);
				...
				attribmatchnum = add_pattern_to_scanning_table(bfparser->st, attribute_name, FALSE, FALSE, TRUE, tagattributecontext, &bfparser->ldb);
			...
			if (values) {
				...
				if (values_arr) {
					...
					valmatchnum = add_pattern_to_scanning_table(bfparser->st,"(\"[^\"]*\"|'[^']*')",TRUE,FALSE,TRUE,valuecontext, &bfparser->ldb);
					pattern_set_runtime_properties(bfparser->st, valmatchnum,"string", -1, FALSE, FALSE,0, FALSE, FALSE);
				}
			}
					 ...
					 gint ih_attrib_autocomplete_backup_cursor, GSList *ih_autocomplete)
{
	...
	if (add_tag) {
		if (idref && idref[0] && !tag) {
			...
			matchnum = add_pattern_to_scanning_table(bfparser->st, tagpattern, FALSE, FALSE, case_insens, context, &bfparser->ldb);
			if (block_name) {
				foldable = lookup_block_foldable(bfparser->bflang->name, block_name);
			}
			...
			if (!contexttag) {
				...
				if (!sgml_shorttag) {
					...
					guint16 tmpnum = add_pattern_to_scanning_table(bfparser->st, "/>", FALSE, FALSE, FALSE, contexttag, &bfparser->ldb);
				...
				startinnertagmatch = add_pattern_to_scanning_table(bfparser->st, ">", FALSE, FALSE, FALSE, contexttag, &bfparser->ldb);
			...
			if (!no_close) {
				...
				endtagmatch = add_pattern_to_scanning_table(bfparser->st, tmp, FALSE, FALSE, case_insens, innercontext, &bfparser->ldb);
...
process_scanning_group(xmlTextReaderPtr reader, Tbflangparsing * bfparser, gint context,
					   ...
					   gint ih_case_insens, gint ih_is_regex, gint ih_is_color, gint ih_attrib_autocomplete_backup_cursor,
		...
		while (xmlTextReaderRead(reader) == 1) {
			...
			if (xmlStrEqual(name, (xmlChar *) "element")) {
				process_scanning_element(reader, bfparser, context, contextstack,
									...
									is_color != UNDEFINED ? is_color : ih_is_color,
...
context_add_indent_detection(Tbflangparsing * bfparser, gint context)
{
	...
	matchnum = add_pattern_to_scanning_table(bfparser->st, "\x02""[ \t]*", TRUE, FALSE, FALSE, context, &bfparser->ldb);
	...
	while (xmlTextReaderRead(reader) == 1) {
		...
		if (xmlStrEqual(name, (xmlChar *) "element")) {
			...
			process_scanning_group(reader, bfparser, context, contextstack, NULL, NULL, NULL, NULL,
								   UNDEFINED, UNDEFINED, UNDEFINED, 0, NULL);
		} else if (xmlStrEqual(name, (xmlChar *) "context")) {
