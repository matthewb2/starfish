...
bfwin_current_document_change_register(Tbfwin *bfwin, CurdocChangedCallback func, gpointer data) {
	callback_register(&bfwin->curdoc_changed, (void (*)())func, data);
}
...
bfwin_document_insert_text_register(Tbfwin *bfwin, DocInsertTextCallback func, gpointer data) {
	callback_register(&bfwin->doc_insert_text, (void (*)())func, data);
}
...
bfwin_document_delete_range_register(Tbfwin *bfwin, DocDeleteRangeCallback func, gpointer data) {
	callback_register(&bfwin->doc_delete_range, (void (*)())func, data);
}
...
{
	callback_register(&bfwin->doc_destroy, (void (*)())func, data);
}
void
...
bfwin_set_title(Tbfwin * bfwin, Tdocument * doc, gint num_modified_change)
{
	...
	if (doc->modified) {
		title = g_strconcat("* ", prfilepart, " - Starfish " VERSION, NULL);
	} else {
		title = g_strconcat(prfilepart, " - Starfish " VERSION, NULL);
	}
 ...
 * doc_still_exists
 *
 * test if a Tdocument pointer still points to a document in this window
 *
 *
 */
gboolean
bfwin_has_document_by_pointer(Tbfwin * bfwin, Tdocument * document)
{
	if (g_list_index(bfwin->documentlist, document)==-1)
		return FALSE;
	return TRUE;
}
