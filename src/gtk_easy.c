 ...
 * gtk_easy.c
 *
 * Copyright (C) 1999-2025 Olivier Sessink
 *
...
color_set_lcb(GtkWidget * widget, gpointer data)
{
	...
	string = gdk_color_to_hexstring(&gcolor);
...
static GtkWidget *hovered_item = NULL;
...
/* Key pressed while hovering over menu item */
static gboolean on_menu_key_press(GtkWidget *menu, GdkEventKey *event, gpointer data) {
	if (!hovered_item || !GTK_IS_MENU_ITEM(hovered_item)) 
		return FALSE;
...
gtk3_enable_dynamic_menu_shortcuts_menuitem(GtkWidget *menuitem, gpointer data) {
	...
	if (submenu && GTK_IS_MENU(submenu)) {
		...
		gtk_container_foreach(GTK_CONTAINER(submenu), gtk3_enable_dynamic_menu_shortcuts_menuitem, data);
	}
}
