...
static void
list_store_set_markup_escaped(GtkListStore *store, GtkTreeIter *iter, gint column, const gchar *text)
{
...
void
fill_output_box(gpointer data, const gchar * string)
{
	GtkTreeIter iter;
	/*int ovector[30]; */
	int nummatches=0;
	...
	if (match && nummatches > 0) {
		...
		if (filename) {
			...
			GFile *addtolist = uri_from_external_input(filename, docparent);
...
static void
outputbox_external_command_lcb(const gchar *output, gpointer bfwin, gpointer data) {
	fill_output_box(OUTPUTBOX(BFWIN(bfwin)->outputbox), output);
}

static void
outputbox_statuscode_lcb(gint status, Tbfwin * bfwin, const gchar *commandstring, gpointer data)
{
	if (status == 0)
		return;
	gchar *tmp =
		g_strdup_printf(_("The command %s exited with error code %d"), commandstring, status);
...
/*	ob->def->show_all_output = show_all_output;*/
	...
	custom_command_lines(bfwin, command, outputbox_statuscode_lcb, outputbox_external_command_lcb, ob);
}
