			...
			gboolean open_in_foreground)
{
	...
	if (!mimetype) {
		...
		DEBUG_MSG("1 WIN32 file_handle: cont_type=%s, mime=%s, mimetype=%s\n",cont_type,mime,mimetype);
#else
...
#ifdef WIN32
		mime = g_content_type_get_mime_type(mimetype);
		DEBUG_MSG("WIN32 file_handle, got mime type %s for mimetype %s\n", mime,mimetype);
#else
		mime = mimetype;
#endif
	}
	...
	if (mime && (
		strcmp(mime, "application/x-bluefish-project") == 0
#ifdef WIN32
		|| strcmp(mime, "application/x-ext-bfproject") == 0
#endif
	)) {
