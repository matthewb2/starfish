...
props_init_main(GHashTable * config_rc)
{
	...
	init_prop_integer(&config_rc, &main_v->props.dynamic_menu_shortcut, "dynamic_menu_shortcut:", 0, TRUE);
...
GList *update_commands(GList *current, gboolean overwrite)
{
	GList *defaults=NULL, *retlist;
#ifdef WIN32
	defaults = add_browser_if_exists(defaults, _("Firefox (x86)"), "C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe", "1");
	defaults = add_browser_if_exists(defaults, _("Firefox"), "C:\\Program Files\\Mozilla Firefox\\firefox.exe", "1");
	defaults = add_browser_if_exists(defaults, _("Internet Explorer (x86)"), "C:\\Program Files (x86)\\Internet Explorer\\iexplore.exe", "0");
	defaults = add_browser_if_exists(defaults, _("Internet Explorer"), "C:\\Program Files\\Internet Explorer\\iexplore.exe", "0");
	defaults = add_browser_if_exists(defaults, _("Edge (x86)"), "C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe", "0");
